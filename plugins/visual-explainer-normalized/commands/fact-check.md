> Normalized skill ID: `visual-explainer:visual-explainer.commands.fact-check`
> Get this skill: `supercli skills get visual-explainer:visual-explainer.commands.fact-check`
...
For HTML files: read `visual-explainer:visual-explainer.references.css-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`) to match the existing page's styling when inserting the verification summary.
