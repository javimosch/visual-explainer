> Normalized skill ID: `visual-explainer:visual-explainer.commands.generate-slides`
> Get this skill: `supercli skills get visual-explainer:visual-explainer.commands.generate-slides`
...
Follow the visual-explainer skill workflow. Read the reference template at `visual-explainer:visual-explainer.templates.slide-deck` (get: `supercli skills get visual-explainer:visual-explainer.templates.slide-deck`) and slide patterns at `visual-explainer:visual-explainer.references.slide-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.slide-patterns`) before generating. Also read `visual-explainer:visual-explainer.references.css-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`) for shared patterns (Mermaid zoom controls, depth tiers, overflow protection) and `visual-explainer:visual-explainer.references.libraries` (get: `supercli skills get visual-explainer:visual-explainer.references.libraries`) for Mermaid theming, Chart.js, and font pairings.
