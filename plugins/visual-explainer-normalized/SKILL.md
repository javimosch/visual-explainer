> Normalized skill ID: `visual-explainer:visual-explainer.skill`
> Get this skill: `supercli skills get visual-explainer:visual-explainer.skill`
...
Detailed prompt templates in `visual-explainer:visual-explainer.commands.*` (list: `supercli skills list --catalog --provider visual-explainer --json`). In Pi, these are slash commands (`/diff-review`). In Claude Code, namespaced (`/visual-explainer:diff-review`). In Codex, use `/prompts:diff-review` (if installed to `~/.codex/prompts/`) or invoke `$visual-explainer` and describe the workflow.
...
For prose accents, see "Prose Page Elements" in `visual-explainer:visual-explainer.references.css-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`). For everything else, use the standard freeform approach with aesthetic directions below.
...
- For text-heavy architecture overviews (card content matters more than topology): read `visual-explainer:visual-explainer.templates.architecture` (get: `supercli skills get visual-explainer:visual-explainer.templates.architecture`)
- For flowcharts, sequence diagrams, ER, state machines, mind maps, class diagrams, C4: read `visual-explainer:visual-explainer.templates.mermaid-flowchart` (get: `supercli skills get visual-explainer:visual-explainer.templates.mermaid-flowchart`)
- For data tables, comparisons, audits, feature matrices: read `visual-explainer:visual-explainer.templates.data-table` (get: `supercli skills get visual-explainer:visual-explainer.templates.data-table`)
- For slide deck presentations (when `--slides` flag is present or `/generate-slides` is invoked): read `visual-explainer:visual-explainer.templates.slide-deck` (get: `supercli skills get visual-explainer:visual-explainer.templates.slide-deck`) and `visual-explainer:visual-explainer.references.slide-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.slide-patterns`)
- For prose-heavy publishable pages (READMEs, articles, blog posts, essays): read the "Prose Page Elements" section in `visual-explainer:visual-explainer.references.css-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`) and "Typography by Content Voice" in `visual-explainer:visual-explainer.references.libraries` (get: `supercli skills get visual-explainer:visual-explainer.references.libraries`)

**For CSS/layout patterns and SVG connectors**, read `visual-explainer:visual-explainer.references.css-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`).

**For pages with 4+ sections** (reviews, recaps, dashboards), also read `visual-explainer:visual-explainer.references.responsive-nav` (get: `supercli skills get visual-explainer:visual-explainer.references.responsive-nav`) for section navigation with sticky sidebar TOC on desktop and horizontal scrollable bar on mobile.
...
**Mermaid theming:** Always use `theme: 'base'` with custom `themeVariables` so colors match your page palette. Use `layout: 'elk'` for complex graphs (requires the `@mermaid-js/layout-elk` package — see `visual-explainer:visual-explainer.references.libraries` (get: `supercli skills get visual-explainer:visual-explainer.references.libraries`) for the CDN import). Override Mermaid's SVG classes with CSS for pixel-perfect control. See `visual-explainer:visual-explainer.references.libraries` (get: `supercli skills get visual-explainer:visual-explainer.references.libraries`) for full theming guide.
...
**Mermaid layout direction:** Prefer `flowchart TD` (top-down) over `flowchart LR` (left-to-right) for complex diagrams. LR spreads horizontally and makes labels unreadable when there are many nodes. Use LR only for simple 3-4 node linear flows. See `visual-explainer:visual-explainer.references.libraries` (get: `supercli skills get visual-explainer:visual-explainer.references.libraries`) "Layout Direction: TD vs LR".
...
See `visual-explainer:visual-explainer.references.css-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`) for image container styles (hero banners, inline illustrations, captions).
...
**Typography is the diagram.** Pick a distinctive font pairing from the list in `visual-explainer:visual-explainer.references.libraries` (get: `supercli skills get visual-explainer:visual-explainer.references.libraries`). Every page should use a different pairing from recent generations.
...
**Text-heavy overviews (under 15 elements):** CSS Grid with explicit row/column placement. Sections as rounded cards with colored borders and monospace labels. Vertical flow arrows between sections. The reference template at `visual-explainer:visual-explainer.templates.architecture` (get: `supercli skills get visual-explainer:visual-explainer.templates.architecture`) demonstrates this pattern. Use when cards need descriptions, code references, tool lists, or other rich content that Mermaid nodes can't hold.
...
Use a real `<table>` element — not CSS Grid pretending to be a table. Tables get accessibility, copy-paste behavior, and column alignment for free. The reference template at `visual-explainer:visual-explainer.templates.data-table` (get: `supercli skills get visual-explainer:visual-explainer.templates.data-table`) demonstrates all patterns below.
...
**Code blocks require explicit formatting.** Without `white-space: pre-wrap`, code runs together into an unreadable wall. See the "Code Blocks" section in `visual-explainer:visual-explainer.references.css-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`) for the correct pattern.
...
Use these sparingly within visual pages to highlight key points or provide breathing room. See "Prose Page Elements" in `visual-explainer:visual-explainer.references.css-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`) for CSS patterns.
...
**Before generating slides**, read `visual-explainer:visual-explainer.references.slide-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.slide-patterns`) (engine CSS, slide types, transitions, nav chrome, presets) and `visual-explainer:visual-explainer.templates.slide-deck` (get: `supercli skills get visual-explainer:visual-explainer.templates.slide-deck`) (reference template showing all 10 types). Also read `visual-explainer:visual-explainer.references.css-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`) for shared patterns and `visual-explainer:visual-explainer.references.libraries` (get: `supercli skills get visual-explainer:visual-explainer.references.libraries`) for Mermaid/Chart.js theming.
...
**Usage:**
```bash
bash {{skill_dir}}/visual-explainer:visual-explainer.scripts.share (get: supercli skills get visual-explainer:visual-explainer.scripts.share) <html-file>
```

**Example:**
```bash
bash {{skill_dir}}/visual-explainer:visual-explainer.scripts.share (get: supercli skills get visual-explainer:visual-explainer.scripts.share) ~/.agent/diagrams/my-diagram.html
...
See `visual-explainer:visual-explainer.commands.share` (get: `supercli skills get visual-explainer:visual-explainer.commands.share`) for the `/share` command template.
...
- **No overflow**: Resize the browser to different widths. No content should clip or escape its container. Every grid and flex child needs `min-width: 0`. Side-by-side panels need `overflow-wrap: break-word`. Never use `display: flex` on `<li>` for marker characters — it creates anonymous flex items that can't shrink, causing lines with many inline `<code>` badges to overflow. Use absolute positioning for markers instead. See the Overflow Protection section in `visual-explainer:visual-explainer.references.css-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`).
- **Mermaid zoom controls**: Every `.mermaid-wrap` container must have zoom controls (+/−/reset/expand buttons), Ctrl/Cmd+scroll zoom, click-and-drag panning, and click-to-expand (clicking without dragging opens the diagram full-size in a new tab). The expand button (⛶) provides the same functionality. See `visual-explainer:visual-explainer.references.css-patterns` (get: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`) for the full pattern including the `openMermaidInNewTab()` function.
...
**Required:** Pick from the font pairings in `visual-explainer:visual-explainer.references.libraries` (get: `supercli skills get visual-explainer:visual-explainer.references.libraries`). Every generation should use a different pairing from the last.
