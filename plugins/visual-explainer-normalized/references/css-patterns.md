> Normalized skill ID: `visual-explainer:visual-explainer.references.css-patterns`
> Get this skill: `supercli skills get visual-explainer:visual-explainer.references.css-patterns`
...
This pattern removes all hardcoded IDs and supports unlimited diagrams per page. For the full implementation (including smart fit, pinch zoom, and shared drag state), use `visual-explainer:visual-explainer.templates.mermaid-flowchart` (get: `supercli skills get visual-explainer:visual-explainer.templates.mermaid-flowchart`) as the canonical source.
